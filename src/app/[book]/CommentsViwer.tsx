...
const CommentsViwer = (props: any) => {
    ...
    const recursiveRendering = (data: any, isSubcomment = false) =>
        data?.map((comment: any) => (
            <div
                key={comment.date}
                className={`h-auto flex flex-col gap-[1rem] self-end w-full relative z-20 ${comment?.last ? "before:h-full before:w-[4rem] before:bg-neutral-900 before:bottom-[-1.2rem] before:left-[-2rem] before:content-[''] before:absolute before:z-[-10]" : ''} `}
                    ...
                    <footer className="flex flex-row justify-between">
                        <section className="flex flex-row gap-5">
                            ...
                            <div className='flex flex-row cursor-pointer gap-2'>
                                <MessageCircle
                                    width={"2rem"}
                                    fill={state.idSubComment === comment.id ? '#fff' : ''}
                                    onClick={() => handleClickSubComment(comment.id)}
                                />
